  ...
  while(data < end && state->pc < pc) {
    ...
    case 0:
      ...
      case DW_CFA_remember_state:
        fs = (ELFFrameStateRegisters *)__builtin_calloc(1,
                                              sizeof(ELFFrameStateRegisters));
        __builtin_memcpy(fs, &state->registers, sizeof(ELFFrameStateRegisters));
      ...
      case DW_CFA_restore_state:
        ...
        __builtin_memcpy(&state->registers, fs, sizeof(ELFFrameStateRegisters));
        free(fs);
        break;
      case DW_CFA_def_cfa:
...
{
  ELFFrameState *state = (ELFFrameState *)__builtin_calloc(1, sizeof(ELFFrameState));
...
void elfPrintCallChain(u32 address)
{
  ...
  __builtin_memcpy(&regs[0], &reg[0], sizeof(reg_pair) * 15);
  ...
  while(count < 20) {
    ...
    if(state->cfaMode == CFA_REG_OFFSET) {
      __builtin_memcpy(&newRegs[0], &regs[0], sizeof(reg_pair) * 15);
      u32 addr = 0;
      for(int i = 0; i < 15; i++) {
      ...
      __builtin_memcpy(regs, newRegs, sizeof(reg_pair)*15);
      addr = newRegs[14].I;
      addr &= 0xfffffffe;
      address = addr;
      count++;
...
u8 *elfReadAttribute(u8 *data, ELFAttr *attr)
{
 ...
 start:
  ...
  case DW_FORM_strp:
    ...
    break;
  case DW_FORM_block:
    attr->block = (ELFBlock *)__builtin_malloc(sizeof(ELFBlock));
...
{
  data += offset;
  ELFAbbrev **abbrevs = (ELFAbbrev **)__builtin_calloc(sizeof(ELFAbbrev *)*121,1);
  int bytes = 0;
  u32 number = elfReadLEB128(data, &bytes);
  data += bytes;
  while(number) {
    ELFAbbrev *abbrev = (ELFAbbrev *)__builtin_calloc(sizeof(ELFAbbrev),1);
  ...
  while(data < end) {
    ...
    if(id == 0xffffffff) {
      ...
      *data++;
      ...
      ELFcie *cie = (ELFcie *)__builtin_calloc(1, sizeof(ELFcie));
      ...
      ELFfde *fde = (ELFfde *)__builtin_calloc(1, sizeof(ELFfde));
...
void elfParseLineInfo(CompileUnit *unit, u8 *top)
{
  ...
  LineInfo *l = unit->lineInfoTable = (LineInfo *)__builtin_calloc(1, sizeof(LineInfo));
  l->number = 0;
  int max = 1000;
  l->lines = (LineInfoItem *)__builtin_malloc(1000*sizeof(LineInfoItem));
  ...
  int lineRange = *data++;
  int opcodeBase = *data++;
  u8 *stdOpLen = (u8 *)__builtin_malloc(opcodeBase * sizeof(u8));
  stdOpLen[0] = 1;
  int i;
  ...
  int count = 4;
  int index = 0;
  l->files = (char **)__builtin_malloc(sizeof(char *)*count);
                  ...
                  Type **type)
{
  ...
  case DW_TAG_structure_type:
    {
      Type *t = (Type *)__builtin_calloc(sizeof(Type), 1);
      if(abbrev->tag == DW_TAG_structure_type)
        t->type = TYPE_struct;
      else
        t->type = TYPE_union;

      Struct *s = (Struct *)__builtin_calloc(sizeof(Struct), 1);
      t->structure = s;
  ...
  case DW_TAG_enumeration_type:
    {
      Type *t = (Type *)__builtin_calloc(sizeof(Type), 1);
      t->type = TYPE_enum;
      Enum *e = (Enum *)__builtin_calloc(sizeof(Enum), 1);
      t->enumeration = e;
      elfAddType(t, unit, offset);
      int count = 0;
      for(int i = 0; i < abbrev->numAttrs; i++) {
  ...
  case DW_TAG_subroutine_type:
    {
      ...
      FunctionType *f = (FunctionType *)__builtin_calloc(sizeof(FunctionType), 1);
      t->function = f;
      elfAddType(t, unit, offset);
      for(int i = 0; i < abbrev->numAttrs; i++) {
        ELFAttr *attr = &abbrev->attrs[i];
  ...
  case DW_TAG_array_type:
    {
      u32 typeref = 0;
      int i;
      Array *array = (Array *)__builtin_calloc(sizeof(Array), 1);
      Type *t = (Type *)__builtin_calloc(sizeof(Type), 1);
      t->type = TYPE_array;
      elfAddType(t, unit, offset);
...
u8 *elfParseObject(u8 *data, ELFAbbrev *abbrev, CompileUnit *unit,
                   Object **object)
{
  Object *o = (Object *)__builtin_calloc(sizeof(Object), 1);
...
u8 *elfParseFunction(u8 *data, ELFAbbrev *abbrev, CompileUnit *unit,
                     Function **f)
{
  Function *func = (Function *)__builtin_calloc(sizeof(Function), 1);
  *f = func;
...
CompileUnit *elfParseCompUnit(u8 *data, u8 *abbrevData)
{
  ...
  CompileUnit *unit = (CompileUnit *)__builtin_calloc(sizeof(CompileUnit), 1);
  unit->top = top;
  unit->length = length;
...
void elfParseAranges(u8 *data)
{
  ...
  int max = 4;
  ARanges *ranges = (ARanges *)__builtin_calloc(sizeof(ARanges), 4);
  ...
  while(data < end) {
    ...
    ranges[index].ranges = (ARange *)__builtin_calloc(sizeof(ARange), (len-20)/8);
    int i = 0;
    while(true) {
...
void elfReadSymtab(u8 *data)
{
  ...
  elfSymbols = (Symbol *)__builtin_malloc(sizeof(Symbol)*count);
...
bool elfReadProgram(ELFHeader *eh, u8 *data, int& size, bool parseDebug)
{
  ...
  for(i = 0; i < count; i++) {
    ...
    if(cpuIsMultiBoot) {
      if(READ32LE(&ph->paddr) >= 0x2000000 &&
        ...
        __builtin_memcpy(&workRAM[READ32LE(&ph->paddr) & 0x3ffff],
      ...
      if(READ32LE(&ph->paddr) >= 0x8000000 &&
        ...
        __builtin_memcpy(&rom[READ32LE(&ph->paddr) & 0x1ffffff],
               data + READ32LE(&ph->offset),
               READ32LE(&ph->filesz));
        size += READ32LE(&ph->filesz);
      }
  ...
  ELFSectionHeader **sh = (ELFSectionHeader **)
    __builtin_malloc(sizeof(ELFSectionHeader *) * count);
    ...
    if(READ32LE(&sh[i]->flags) & 2) { // load section
      if(cpuIsMultiBoot) {
        if(READ32LE(&sh[i]->addr) >= 0x2000000 &&
          ...
          __builtin_memcpy(&workRAM[READ32LE(&sh[i]->addr) & 0x3ffff], data +
        ...
        if(READ32LE(&sh[i]->addr) >= 0x8000000 &&
          ...
          __builtin_memcpy(&rom[READ32LE(&sh[i]->addr) & 0x1ffffff],
  ...
  if(parseDebug) {
    ...
    elfDebugInfo = (DebugInfo *)__builtin_calloc(sizeof(DebugInfo), 1);
...
bool elfRead(const char *name, int& siz, FILE *f)
{
  fseek(f, 0, SEEK_END);
  long size = ftell(f);
  elfFileData = (u8 *)__builtin_malloc(size);
  fseek(f, 0, SEEK_SET);
