...
void Blip_Buffer::clear( int entire_buffer )
{
	...
	if ( buffer_ )
	{
		...
		XMemSet( buffer_, 0, (count + blip_buffer_extra_) * sizeof (buf_t_) );
	}
}
...
void Blip_Buffer::remove_samples( long count )
{
	if ( count )
	{
		...
		XMemSet( buffer_ + remain, 0, count * sizeof *buffer_ );
	}
}
...
void Blip_Buffer::save_state( blip_buffer_state_t* out )
{
	...
	XMemCpy( out->buf, &buffer_ [offset_ >> BLIP_BUFFER_ACCURACY], sizeof out->buf );
}
...
void Blip_Buffer::load_state( blip_buffer_state_t const& in )
{
	...
	XMemCpy( buffer_, in.buf, sizeof in.buf );
}
