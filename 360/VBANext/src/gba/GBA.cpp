...
#if (0)
static bool CPUXboxWriteState(HANDLE file)
{
	variable_desc *data;
	xStorage.WriteStream(file,(char *)SAVE_GAME_VERSION,sizeof(int));
	xStorage.WriteStream(file,(char *)&rom[0xa0],16);
	xStorage.WriteStream(file,(char *)&reg[0], sizeof(reg));

	data = saveGameStruct;
	while(data->address) {	 
		xStorage.WriteStream(file,(char *)data->address,data->size);
		data++;
	}

	xStorage.WriteStream(file,(char *)stopState,sizeof(int));
	xStorage.WriteStream(file,(char *)IRQTicks,sizeof(int));

	xStorage.WriteStream(file,(char *)internalRAM,0x8000);

	xStorage.WriteStream(file, (char *)paletteRAM, 0x400);
	xStorage.WriteStream(file, (char *)workRAM, 0x40000);
	xStorage.WriteStream(file, (char *)vram, 0x20000);
	xStorage.WriteStream(file, (char *)oam, 0x400);
	xStorage.WriteStream(file, (char *)pix, 4*241*162);
	xStorage.WriteStream(file, (char *)ioMem, 0x400);
 
	Xbox_eepromSaveGame(file);
	Xbox_flashSaveGame(file);
	Xbox_soundSaveGame(file);

	Xbox_cheatsSaveGame(file);

	// version 1.5
	Xbox_rtcSaveGame(file);
...
bool CPUWriteBatteryFile(const char *fileName)
{
	...
	sprintf_s(srmName, fileName);
	swprintf_s(romName, L"%S", fileName);
  ...
  if((gbaSaveType) && (gbaSaveType!=5)) {   
    ...
    if(gbaSaveType != 3) 
	{
      if(gbaSaveType == 2) 
	  {	    
        xStorage.WriteSaveGame(srmName,romName, (char *)flashSaveMemory,flashSize);                  
      } 
	  else 
	  {
	    xStorage.WriteSaveGame(srmName,romName, (char *)flashSaveMemory,0x10000);		 
      }
    } 
	else 
	{
		xStorage.WriteSaveGame(srmName,romName, (char *)eepromData,eepromSize);     
    }
                         ...
                         const char *notes)
{
  ...
  XMemSet(temp, 0, 28);
  XMemCpy(temp, &rom[0xa0], 16); // copy internal name
  ...
  temp[0x14] = 1; // 1 save ?
  XMemCpy(&temp[0x1c], flashSaveMemory, saveSize); // copy save
...
bool CPUReadBatteryFile(const char *fileName)
{
  ...
  HANDLE file = xStorage.OpenStreamForRead(srmName,romName,&xov,&hEventComplete);
  ...
  if (file)
  {
	  ...
	  if(size == 512 || size == 0x2000) {
		  ...
		  if (xStorage.ReadStream(file,(char *)&eepromData,size) != (size_t)size)
		  {
			xStorage.CloseStream(file, &xov);
			CloseHandle(hEventComplete);
			return false;
		  }    
	  } else {
		if(size == 0x20000) {
		  if (xStorage.ReadStream(file,(char *)&flashSaveMemory,0x20000) != 0x20000)
		  {
			xStorage.CloseStream(file, &xov);
			CloseHandle(hEventComplete);
		  }
		  ...
		  if (xStorage.ReadStream(file,(char *)&flashSaveMemory,0x10000) != 0x10000)
		  {
			xStorage.CloseStream(file, &xov);
			CloseHandle(hEventComplete);
		  }
...
int CPULoadRom(const char *szFile)
{
  ...
  rom = (u8 *)XPhysicalAlloc(0x2000000, MAXULONG_PTR, 0,
...
void doMirroring (bool b)
{
  ...
  if ((mirroredRomSize <=0x800000) && (b))
  {
    ...
    {
      XMemCpy ((u16 *)(rom+mirroredRomAddress), (u16 *)(rom), mirroredRomSize);
      mirroredRomAddress+=mirroredRomSize;
    }
  }
}
...
#ifdef WORDS_BIGENDIAN
 __inline void CPUSwap(volatile u32 * __restrict a, volatile u32 * __restrict b)
{
  volatile u32 c = *b;
  *b = *a;
  *a = c;
...
void CPUInit(const char *biosFileName, bool useBiosFile)
{
  ...
  if(!useBios) {
    XMemCpy(bios, myROM, sizeof(myROM));
  }
...
void CPUReset()
{
  ...
  rtcReset();
  // clean registers
  XMemSet(&reg[0], 0, sizeof(reg));
  // clean OAM
  XMemSet(oam, 0, 0x400);
  // clean palette
  XMemSet(paletteRAM, 0, 0x400);
  // clean picture
  XMemSet(pix, 0, 4*160*240);
  // clean vram
  XMemSet(vram, 0, 0x20000);
  // clean io memory
  XMemSet(ioMem, 0, 0x400);
        ...
        if(DISPSTAT & 1) { // V-BLANK
          ...
          if(DISPSTAT & 2) {
            ...
            if(frameCount >= framesToSkip)
            {
              ...
              /*  case 16:
                {
                  u16 * __restrict dest = (u16 *)pix + 242 * (VCOUNT+1);
                  for(int x = 0; x < 240;) {
                ...
                case 24:
                {
                  u8 * __restrict dest = (u8 *)pix + 240 * VCOUNT * 3;
                  for(int x = 0; x < 240;) {
                ...
                //case 32:
                //{
                  u32 * __restrict dest = (u32 *)pix + 241 * (VCOUNT+1);
                  for(int x = 0; x < 240; ) {
