...
std::string hbkbd::keyboard(const std::string& suggestion)
{
    ...
    while (appletMainLoop() && !(hidKeysDown(CONTROLLER_P1_AUTO) & KEY_B))
    {
        ...
        memset(currentFB, 51, gfxGetFramebufferSize());

        drawRect(marginlr, 140, 1280 - marginlr*2, 84, COLOR_GREY_MEDIUM);
        drawRect(0, starty - margintb, 1280, 356, COLOR_GREY_DARKER);
