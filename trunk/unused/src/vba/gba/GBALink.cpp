...
char *MakeInstanceFilename(const char *Input)
{
	...
	result = (char *)__builtin_malloc(strlen(Input)+3);
			...
			case RFU_COMM:
				...
				else if (a == 0x8000)
				{
					...
					case 0xa7:	//	2nd part of wait function 0x27
						...
						if ( linkid > 0) {
							__builtin_memcpy(rfu_masterdata, linkmem->rfu_data[1-vbaid], 128);
...
void lserver::Recv(void){
	...
	if(lanlink.type==0){	// TCP
		...
		for(i=0;i<lanlink.numgbas;i++){
			...
			if(howmanytimes>1) __builtin_memcpy(inbuffer, inbuffer+inbuffer[0]*(howmanytimes-1), inbuffer[0]);
			if(inbuffer[1]==-32){
				char message[30];
				lanlink.connected = false;
