...
static __inline blip_time_t blip_time()
{
...
#if (0)
void Xbox_soundSaveGame( HANDLE out )
{
	...
	data = gba_state;
	while(data->address) {	 
		xStorage.WriteStream(out,(char *)data->address,data->size);
		data++;
	}
...
static void soundReadGameOld( gzFile in, int version )
{
	...
	XMemCpy( &state.apu.regs [0x20], &ioMem [0x90], 0x10 );
	XMemCpy( &state.apu.regs [0x30], &ioMem [0x90], 0x10 );
