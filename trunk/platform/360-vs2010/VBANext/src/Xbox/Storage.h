...
public:
    ...
    unsigned long WriteSaveGame( CONST CHAR* szFileName, CONST WCHAR* szDisplayName, CHAR *szBuffer, DWORD bytesToWrite );	
    unsigned long ReadSaveGame( XCONTENT_DATA* ContentData, char *szBuffer, DWORD bytesToRead, DWORD *bytesRead );
	...
	unsigned long WriteStream(HANDLE hFile, CHAR *szBuffer, DWORD bytesToWrite);
	unsigned long ReadStream(HANDLE hFile, CHAR *szBuffer, DWORD bytesToRead);
    ...
    static int     IsUserSignedIn( DWORD dwController )
    {
    ...
    static int     IsUserOnline( DWORD dwController )
    {
