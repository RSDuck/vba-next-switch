...
__inline const char* Gb_Apu::save_load( gb_apu_state_t* io, bool save )
{
	#if !GB_APU_CUSTOM_STATE
	...
	if ( save )
		XMemCpy( io->regs, regs, sizeof io->regs );
	else
		XMemCpy( regs, io->regs, sizeof     regs );
...
__inline void Gb_Apu::save_load2( gb_apu_state_t* io, bool save )
{
