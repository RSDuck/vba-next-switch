...
void RingBuffer<T>::read(T *out, std::size_t num) {
	if (rpos + num > sz) {
		...
		__builtin_memcpy(out, buf + rpos, n * sizeof(T));
	...
	__builtin_memcpy(out, buf + rpos, num * sizeof(T));
...
void RingBuffer<T>::write(const T *in, std::size_t num) {
	if (wpos + num > sz) {
		...
		__builtin_memcpy(buf + wpos, in, n * sizeof(T));
	...
	__builtin_memcpy(buf + wpos, in, num * sizeof(T));
